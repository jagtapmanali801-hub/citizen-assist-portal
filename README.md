...
The website's normal service search does **not** call Gemini. It searches the local service data in the browser, so it does not consume Gemini API quota. Only chatbot messages are sent to the Gemini API. Google free-tier quotas/rate limits apply; this is not an unlimited API. Do not enable paid billing unless you intentionally want paid API usage.
...
The chatbot uses the Netlify Function at `/.netlify/functions/chatbot` and the Gemini `gemini-2.5-flash` model. Free-tier quotas/rate limits apply.
